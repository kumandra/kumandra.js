...
/**
 * @name KumandraClient
 * @summary  The KumandraClient class expose methods to
 * Put and Get an Object from Kumandra network using the Storage API.
 */
export class KumandraClient {
  /**
   * @name connect
   * @summary Connect to Kumandra network RPC nodes to put and get objects using the Kumandra Storage API.
   * @param identity A previously created Identity Object.
   * @param wsRpcEndpoint Optional, by default it will connect to a public Kumandra client rpc node.
   * @param wsRpcEndpointFarmer Optional, by default it will connect to a public Kumandra farmer rpc node.
   * @return Promise<KumandraClient>
   */
  public static async connect(
    identity: Identity,
    wsRpcEndpoint?: string,
    wsRpcEndpointFarmer?: string
  ): Promise<KumandraClient> {
    ...
    return new KumandraClient(api, farmerProvider, identity);
  }
